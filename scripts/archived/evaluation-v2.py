...
def get_recommendations_for_group(recipes_df, tag_column, recipe_index, top_n=10):
    ...
    # Ensure valid features exist
    if recipes_df[f'{tag_column}_features'].str.strip().eq('').all():
        raise ValueError(f"No valid features for tag column: {tag_column}")
    ...
    # Return a DataFrame with similarity scores
    return pd.DataFrame({
...
def evaluate_recommendation_system(
    ...
    for group in tag_groups:
        ...
        try:
            ...
            for recipe_index in recommendations.index:
                matching_recipes = recipes_df.iloc[recipe_index]
                recipe_name = matching_recipes['name']
                print(f"  - Recipe Index {recipe_index}: {recipe_name}")
